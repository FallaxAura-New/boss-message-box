...
本版本使用新增的 `0007_live_routing.sql`。它只新增分流列、直播批次、展示快照、导入任务及审计记录，不修改 `0001`–`0006`，不清空留言、回复、管理员或图片。旧的 AI 保留且未回复留言默认进入“待分流”；已回复留言不会自动取得直播资格。部署前应单独审阅远程已应用的迁移，特别是旧的 `0004` 含历史重置操作，不能把全套旧迁移当作安全升级脚本重新执行。
...
- `source_type`：`public / imported`；Excel 数据只在 `live_entries`，没有虚构的公开身份或手机号。
...
开播前按原始提交时间、稳定 ID 排序；第一次进入直播模式后固定已有队列，之后新选入的留言追加队尾，即使其原始时间更早。Excel 以任务创建时间记入来源时间，同一任务按原始 Excel 行号播放（任务预留队列位置，AI 完成先后不改变行序）。右方向键前进、左方向键回看；直播只读当前批次，不读普通未回复或待办。批次 ID 参与详情与相邻缓存键，批次/成员版本变化时清空缓存；活动画面每 4 秒核对版本，空批次每 5 秒等待新留言，保留快速连按、预加载、图片预热、自动隐藏按钮与抠像背景。
...
- 同批次文件哈希去重，任务 ID 与有效行内容哈希校验冲突，任务 ID + 行号唯一；重试只重新处理失败行，不重复创建展示记录。页面刷新后可从任务选择器或 `job` URL 恢复进度。
- 批次归档会立即将尚未完成的行标记为 `batch_archived`；迟到的 AI 响应不能写入旧批次或新批次。如需继续，在当前批次重新选择文件，历史失败记录保留。
