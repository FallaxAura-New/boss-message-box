...
本版本使用新增的 `0007_live_routing.sql`、`0008_import_routing.sql` 与 `0009_live_cancel_returns_to_routing.sql`。`0007` 新增分流列、直播批次、展示快照、导入任务及审计记录；`0008` 为 Excel 行补充 AI 分类与人工分流状态，并把新导入的自动入播改成人工选入；`0009` 让取消直播展示的观众留言和 Excel 行退回待分流，并允许 Excel 行在退回后重新选入当前或后续批次。迁移不清空留言、回复、管理员或图片；升级前已进入直播批次的 Excel 行继续保留原批次与历史状态。旧的 AI 保留且未回复留言默认进入“待分流”；已回复留言不会自动取得直播资格。部署前应单独审阅远程已应用的迁移，特别是旧的 `0004` 含历史重置操作，不能把全套旧迁移当作安全升级脚本重新执行。
...
开播前按原始提交时间、稳定 ID 排序；第一次进入直播模式后固定已有队列，之后新选入的留言追加队尾，即使其原始时间更早。Excel 以任务创建时间记入来源时间；开播前选入的同一任务按原始 Excel 行号播放，AI 完成先后不改变行序；开播后则按管理员实际选入顺序追加。右方向键前进、左方向键回看；直播只读当前批次，不读普通未回复或待办。批次 ID 参与详情与相邻缓存键，批次/成员版本变化时清空缓存；活动画面每 4 秒核对版本，空批次每 5 秒等待新留言，保留快速连按、预加载、图片预热、自动隐藏按钮与抠像背景。普通模式下，侧栏左下的“直播展示模式”在所有 Studio 页面都可进入当前活动批次，历史批次选择不会限制这个全局入口。
...
验证命令：`pnpm run check`；另启动本机预览后执行 `VISUAL_BASE_URL=http://127.0.0.1:5173 pnpm run check:visual:live`，检查 375 / 768 / 1280px 的分流、资格标记、导入预览、进度、历史、刷新确认和直播画面。界面检查使用明确的模拟数据，实际 XLSX 解析与 D1 事务另由单元及 Worker 集成测试覆盖，不等同于线上 AI 或远程部署验证。

直播正文会铺满可用正文列，并按窗口宽高、实际换行和内容长度测量字号；短文自动放大，长文达到可读字号下限后可滚动读完。手机竖屏将图片放在正文下方，桌面和短横屏保留右侧图片；昵称与主题整体可聚焦滚动。运行 `VISUAL_BASE_URL=http://127.0.0.1:5173 pnpm run check:visual:responsive` 可验证 375–2654px 宽度、短横屏、连续缩放、200% 文字缩放及导入详情的打开、刷新、返回。这次界面修复无需新增数据库迁移，更新部署后刷新页面即可使用。
